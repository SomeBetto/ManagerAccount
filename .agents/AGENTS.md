# Guía de Reglas y Directivas del Proyecto (ManagerAccount - Flyff)

Este documento define la arquitectura, convenciones, directivas de desarrollo e instrucciones especiales para asistentes de Inteligencia Artificial (IA) y desarrolladores que trabajen en este repositorio.

---

## 1. Pruebas en Navegador y Ejecución
- 🚫 **NO realizar pruebas automáticas en el navegador (`browser_subagent`)**.
- El usuario realiza la verificación de interfaz y funcionamiento manualmente en su propio navegador.
- La IA debe limitarse a aplicar las modificaciones solicitadas en el código de manera directa, limpia y eficiente, informando al usuario una vez completadas las ediciones.

---

## 2. Gestión de OTP y Autenticación
- 🛡️ **`otp_token` (OTP de Respaldo)**: Clave/token estático asignado a la cuenta que permite su desvinculación. Debe conservarse intacto en el Excel y la base de datos.
- 🔑 **`totp_secret` (Secreto OTP Dinámico)**: Clave Base32 utilizada para generar dinámicamente códigos TOTP de 6 dígitos (RFC 6238 HMAC-SHA1).
- 🏷️ **Tarjetas de Cuentas y Personajes**: Mostrar la opción de copiar el código dinámico **TOTP**. En las tarjetas de personajes y favoritos, solo mantener la opción de copiar TOTP.

---

## 3. Visibilidad de Botones de Encabezado Principal (`header`)
- El botón **+ Nueva Cuenta** (`#btn-add-account`) se muestra **ÚNICAMENTE** cuando la vista activa es `accounts`.
- El botón **+ Nuevo Personaje** (`#btn-add-character`) se muestra **ÚNICAMENTE** cuando la vista activa es `characters`.
- En el resto de secciones (Dashboard, Generador OTP, Contadores, Coupons, etc.) ambos botones deben permanecer ocultos.

---

## 4. Persistencia y Base de Datos (Excel)
- **Backend de Excel (`backend/app/excel_db.py`):**
  ...
  - **Concurrencia y Auto-Recuperación (`threading.Lock`):**
    - Las escrituras en Excel cuentan con protección mediante `threading.Lock` para evitar conflictos cuando OneDrive sincroniza el archivo.
    - Las actualizaciones múltiples de datos deben realizarse por lotes (Batch) en lugar de peticiones HTTP paralelas simultáneas.
    - En caso de errores de CRC-32 o corrupción de zip por OneDrive, `_get_workbook()` ejecuta una auto-restauración transparente del último backup limpio.
...
## 5. Catálogo Unificado y Reglas de Equipamiento
...
## 6. Registro Físico de Errores (Logging System)
...
## 9. Estilos, UX y Atajos de Teclado
